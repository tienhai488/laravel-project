...
@section('title', 'Cập nhập khóa học')
@section('content')
    @if ($errors->any())
    ...
    <form class="row" method="POST">
        ...
        <div class="form-group col-6">
            ...
            <input name="slug" type="text"
                ...
                placeholder="Slug..." value="{{ old('slug') ?? $course->slug }}" />
            @error('slug')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="form-group col-6">
            ...
            <select name="teacher_id"
                ...
                <option value="1" {{ (old('teacher_id') ?? $course->teacher_id) == 1 ? 'selected' : false }}>TienHai
                </option>
            </select>
            @error('teacher_id')
                <div class="invalid-feedback">
        ...
        <div class="form-group col-6">
            ...
            <input name="price" type="text"
                ...
                placeholder="Giá khóa học..." value="{{ old('price') ?? $course->price }}" />
            @error('price')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="form-group col-6">
            ...
            <input name="sale_price" type="text"
                ...
                placeholder="Giá khuyến mãi..." value="{{ old('sale_price') ?? $course->sale_price }}" />
            @error('sale_price')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="form-group col-6">
            ...
            <select name="is_document"
                ...
                <option value="0" {{ (old('is_document') ?? $course->is_document) == 0 ? 'selected' : false }}>Không
                </option>
                <option value="1" {{ (old('is_document') ?? $course->is_document) == 1 ? 'selected' : false }}>Có
                </option>
            </select>
            @error('is_document')
                <div class="invalid-feedback">
        ...
        <div class="form-group col-6">
            ...
            <select name="status"
                ...
                <option value="1" {{ (old('status') ?? $course->status) == 1 ? 'selected' : false }}>Ra mắt</option>
            </select>
            @error('status')
                <div class="invalid-feedback">
                    {{ $message }}
        ...
        <div class="form-group col-12">
            ...
            <textarea class="form-control editor" placeholder="Hỗ trợ học viên..." name="support" cols="30" rows="10">{{ old('support') ?? $course->support }}</textarea>
            @error('support')
        ...
        <div class="form-group col-12">
            ...
            <textarea class="form-control editor" placeholder="Mô tả khóa học..." name="detail" cols="30" rows="10">{{ old('detail') ?? $course->detail }}</textarea>
            @error('detail')
                <div class="invalid-feedback" style="display: contents">
                    {{ $message }}
                </div>
                ...
                @foreach ($categories as $cate)
                    <div>
                        <input type="checkbox" id="cate-{{ $cate->id }}" class="" name="categories[]"
                            value="{{ $cate->id }}"
                            {{ checkCategory($cate->id, old('categories') ? old('categories') : $dataCate) ? 'checked' : false }}>
